	...
	const int H, int W, int D)
{
	...
	int rendered;
    rendered = OccAggregator::Aggregator::forward(
		geomFunc,
		binningFunc,
		imgFunc,
		P, N,
	...
	const torch::Tensor& bin_logits_grad)
{
	...
	OccAggregator::Aggregator::backward(
		P, R, N,
		H, W, D,
	...
	return std::make_tuple(means3D_grad, opas_grad, cov3D_grad);
}
