...
from ..encoder.gaussianformer.utils import (
    safe_get_quaternion, batch_quaternion_multiply, safe_sigmoid, safe_norm, safe_inverse_sigmoid, safe_inverse_norm)
from ..segmentor.gaussian_segmentor.utils import ConvertData, render_sampled_points_to_image, project_points
...
class ChannelLifterLayer(nn.Module):
    def __init__(self, embed_dim):
        super(ChannelLifterLayer, self).__init__()
...
class GaussianLifter(CustomSerialization):
    def __init__(
            self,
            embed_dims,  # 96
        ...
        if not self.flag_depthbranch:
            # self.channel_lifter = ChannelLifterLayer(embed_dims)
            self.channel_lifter = DepthAwareLayer(embed_dims)
    ...
    def init_curr_anchor(self, xyz_cam, metas):
        device = xyz_cam.device
        init_xyz = xyz_cam  # [bs, anchor_num, 3]

        bs, init_anchor_num, _ = init_xyz.shape
        init_scale = torch.rand_like(init_xyz).to(device)  # [bs, anchor_num, 3]
        init_scale = safe_inverse_sigmoid(init_scale)

        world2cam = metas[0]['world2cam'].to(torch.float32)
        w2c_rot = world2cam[:, :3, :3]  # [B, 3, 3]
        w2c_quat = safe_get_quaternion(w2c_rot)  # [B, 4]
        init_rots = torch.zeros(bs, init_anchor_num, 4, dtype=torch.float).to(device)
        init_rots[..., 0] = 1
        init_rots = batch_quaternion_multiply(w2c_quat, init_rots)  # [bs, num, 4]
        init_opacity = safe_inverse_sigmoid(
            0.1 * torch.ones((bs, init_anchor_num, int(True)), dtype=torch.float).to(device)
        )
        init_semantic = torch.randn(bs, init_anchor_num, self.semantic_dim, dtype=torch.float).to(device)
        # [bs, anchor_num, 23]

        curr_anchor = torch.cat(
            [init_xyz, init_scale, init_rots, init_opacity, init_semantic], dim=-1).to(device)
    ...
    def forward(
        ...
        epsilon = 1e-6
        # self.convert_data.convert_camera_info(cam2world, cam_k, 'camera')
        ...
        depth_feat_flatten = depth_feat.permute(0, 2, 3, 1).view(bs_, -1, dim_)  # [B, h * w, feat dim]
        ...
        if self.sample_mode == 'nearest':
            ...
            depth_feat_sampled = depth_feat_flatten[:, flatten_index_map]  # [B, num anchor, 128] or [B, num anchor, 64]
            depth_pred_sampled = da_pred_flatten[:, flatten_index_map]  # [B, num anchor, 1]
            if depth_conf_flatten is not None:
                depth_conf_sampled = depth_conf_flatten[:, flatten_index_map]  # [B, num anchor]
            else:
            ...
            # cam anchors
            da_pts_cam_t = self.pix2cam(depthnet_output, cam_k)  # [H*W, 3]
        ...
        # cam anchors
        # da_pts_cam_t = self.pix2cam(depthnet_output, cam_k)  # [hw, 3]
        ...
        # self.convert_data.convert_depth(depthnet_output, 'depth')
        # self.convert_data.convert_da_pts_color(da_pts_world_t, color_sampled, 'coz_world')

        """ 2. Depth Branch """

        inference_feature = depth_feat_sampled  # [B, 128, h, w], or [B, num anchor, 128]
        conf_map = depth_conf_sampled  # [B, h, w] or [B, num anchor] or None
        ...
        if flag_depthbranch:
            ...
            if self.fine_tune_mode == 'scale':
                ...
                da_pred_scaled = depthnet_output.unsqueeze(0) * (1 + scale_factor)  # [1, h, w]
                ...
                refined_depth_map = self.depth_head(output_feature)  # [B, 1, h, w]
            ...
            # da_pred_scaled = depth_pred_sampled  # [bs, num anchor, 1]
            # da_pts_cam_sampled_scaled = self.pix2cam(da_pred_scaled, cam_k)
            ...
            # inference_feature = self.channel_lifter(inference_feature)
            inference_feature = self.channel_lifter(depth_pred_sampled)  # [bs, num anchor, 96]
            da_pred_scaled = da_pred_scaled.reshape(bs_, self.sample_shape[0], self.sample_shape[1])  # [bs, h, w]

        # self.convert_data.convert_depth(da_pred_scaled.squeeze(0), 'pf_depth')
        ...
        """ 3. Initialize Gaussian Primitives """

        gaussian_anchor = self.init_curr_anchor(da_pts_cam_valid, metas)
        ...
        # return da_pred_scaled, da_pts_cam_sampled_scaled.squeeze(0), da_pts_world_sampled_scaled.squeeze(0), flatten_index_map, gaussian_anchor, inference_feature, conf_map
        # return da_pred_scaled_valid, da_pts_cam_valid.squeeze(0), da_pts_world_valid.squeeze(0), flatten_index_map_valid, gaussian_anchor, inference_feature_valid, conf_map_valid
        return da_pred_scaled_valid, da_pts_cam_valid, da_pts_world_valid, flatten_index_map_valid, gaussian_anchor, inference_feature_valid, conf_map_valid
