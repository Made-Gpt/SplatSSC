...
try:
    ...
    raise ImportError("The `_C` module could not be imported. Ensure it is built correctly.") from e
...
class _LocalAggregate(torch.autograd.Function):
    ...
    def backward(
        ...
        W = ctx.W
        D = ctx.D
        geomBuffer, binningBuffer, imgBuffer, means3D, pts, points_int, cov3D, opas, opas_thresh, semantics, logits, bin_logits, density, probability = ctx.saved_tensors
...
class LocalAggregator(nn.Module):
    ...
    def forward(
        ...
        assert opas_thresh is not None, "value 'opas_thresh' please!"
        ...
        # geo_hit = torch.unique(means3D_int, dim=0) * self.grid_size + self.pc_min + self.grid_size / 2  # [num, 3]
