...
{
    auto idx = cg::this_grid().thread_rank();  // index of current thread in the whole grid
	if (idx >= N)
	    return;
	...
	float C[CHANNELS] = {0};  // only valid dims
	float bin_logit = 1.0;
	float density = 0.0;
	float prob_sum = 0.0;
	...
	for (int i = range.x; i < range.y; i++)
	{
		...
		// My Fix: gate control
		//  if (opas[gs_idx] < opas_thresh){
		//     power = exp(power) * opas[gs_idx];
		// } else {
		//     power = exp(power);
		// }

		float deter = cov1.x * cov1.y * cov1.z + 2 * cov2.x * cov2.y * cov2.z - cov1.x * cov2.y * cov2.y - cov1.y * cov2.z * cov2.z - cov1.z * cov2.x * cov2.x;  // covariance matrix in formular 7
		...
		// My Fix
		// bin_logit = (1 - power) * bin_logit;  // probability of not occupied by gaussian G_i (empty)
		bin_logit = (1 - power * opas[gs_idx]) * bin_logit;  // probability of not occupied by gaussian G_i (empty)

		density = power + density;  // probability of occupied by gaussian G_i
		prob_sum = prob + prob_sum;  // summation of conditional probability
	}
	...
	if (prob_sum > epsilon) {
		...
		for (int ch = 0; ch < CHANNELS; ch++)
		{
		    out_logits[idx * CHANNELS + ch] = 1.0 / (CHANNELS);
		}

	}

	out_bin_logits[idx] = 1 - bin_logit;  // formulate (5), probability of occupied
