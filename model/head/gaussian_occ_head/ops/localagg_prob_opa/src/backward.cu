	...
	float* __restrict__ cov3D_grad)
{
	...
	for (int i = start; i < end; i++)
	{
		...
		if (pts_idx >= 0)
		{
		    // key intermediate value during forward
			...
			if (prob_sum > epsilon) {
				for (int ch = 0; ch < CHANNELS; ch++)
				{
					...
					opa_grad += logits_grad[pts_idx * CHANNELS + ch] * (sem[ch] - logits[pts_idx * CHANNELS + ch]) * prob / prob_sum;  // grad from 'out_logit'
				}
			}
			...
			power_grad += prob_grad * powf(2 * 3.1415926535, -1.5) * powf(deter, 0.5);  // grad from 'out_logit'
			power_grad += bin_logits_grad[pts_idx] * other_bin_logit * opa;  // grad from 'out_bin_logit'
			power_grad += density_grad[pts_idx];  // grad from 'density'
            opa_grad += bin_logits_grad[pts_idx] * other_bin_logit * power;  // grad from 'out_bin_logit'

			// if (opa < opas_thresh) {
			//     opa_grad += power_grad * power;
			//     power_grad *= opa;
			// }
