...
class LocalAggregator(nn.Module):
    def __init__(self, scale_multiplier, H, W, D, pc_min, grid_size, inv_softmax=False, scale_range=[0.01, 0.16]):
        super().__init__()
    ...
    def forward(
        ...
        # make sure point index limited to H, W, D
        ...
        assert means3D_int.min() >= 0, f'{means3D_int.min()} is smaller than 0'
        assert means3D_int[:, 0].max() < self.H, f'{means3D_int[:, 0].max()} is larger than {self.H}'
        assert means3D_int[:, 1].max() < self.W, f'{means3D_int[:, 1].max()} is larger than {self.W}'
        assert means3D_int[:, 2].max() < self.D, f'{means3D_int[:, 2].max()} is larger than {self.D}'
        ...
        # recover to world coordinate
        # geo_hit = torch.unique(means3D_int, dim=0) * self.grid_size + nyu_pc_min + self.grid_size / 2  # [num, 3]
