	...
	const int H, int W, int D)
{
	...
	const int P = means3D.size(0);  // number of gaussian points
	const int N = pts.size(0);  // number of gt_xyz, 129600 <-- 60 * 60 * 36

	auto int_opts = means3D.options().dtype(torch::kInt32);
	auto float_opts = means3D.options().dtype(torch::kFloat32);

    // the final output, it is a 2D tensor, [anchor num, class num]
	...
	const torch::Tensor& out_grad) 
{
	...
	torch::Tensor semantics_grad = torch::zeros({P, valid_channels}, means3D.options());
