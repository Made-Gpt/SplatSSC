...
int LocalAggregator::Aggregator::forward(
	...
	bool debug)
{
	size_t chunk_size = required<GeometryState>(P);  // get memory size of storing P gaussian points
	char* chunkptr = geometryBuffer(chunk_size);  // dynamically allocate buffer
	GeometryState geomState = GeometryState::fromChunk(chunkptr, P);  // initialize
	...
	// get touched vox for each gaussian anchor
