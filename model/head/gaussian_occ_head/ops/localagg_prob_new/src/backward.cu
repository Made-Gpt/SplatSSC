	...
	float* __restrict__ cov3D_grad)
{
	...
	for (int i = start; i < end; i++)
	{
		...
		if (pts_idx >= 0)
		{
			...
			if (prob_sum > epsilon)
			{
			    float acc_sem_j = 0.0f;  // Σ(dL/de * e)
                float sem_sum_j = 0.0f;  // Σ(sem_i)
                for (int ch = 0; ch < CHANNELS; ch++)
                {
                    acc_sem_j += logits_grad[pts_idx * CHANNELS + ch] * logits[pts_idx * CHANNELS + ch];
                    sem_sum_j += sem[ch];
                }

                float prob_grad_from_logits = 0.0f;
			    for (int ch = 0; ch < CHANNELS; ch++)
			    {
			        // semantic grad
			        // semantic_grad[ch] += logits_grad[pts_idx * CHANNELS + ch] * logits[pts_idx * CHANNELS + ch] * prob / prob_sum;
			        // semantic_grad[ch] += (prob / prob_sum) * (logits_grad[pts_idx * CHANNELS + ch] - acc_sem);
			        // semantic_grad[ch] += logits_grad[pts_idx * CHANNELS + ch] * prob * (1 - logits[pts_idx * CHANNELS + ch]) / prob_sum;
                    semantic_grad[ch] += prob * (logits_grad[pts_idx * CHANNELS + ch] - acc_sem_j) / prob_sum;

			        // prob grad
			        // prob_grad += logits_grad[pts_idx * CHANNELS + ch] * (sem[ch] - logits[pts_idx * CHANNELS + ch]) / prob_sum;
			        // prob_grad += logits_grad[pts_idx * CHANNELS + ch] * (sem[ch] - logits[pts_idx * CHANNELS + ch] * sem_sum);
			        // prob_grad += logits_grad[pts_idx * CHANNELS + ch] * sem[ch] * (1 - logits[pts_idx * CHANNELS + ch]) / prob_sum;
			        prob_grad_from_logits += logits_grad[pts_idx * CHANNELS + ch] * (sem[ch] - logits[pts_idx * CHANNELS + ch] * sem_sum_j);
			    }
			    // prob_grad /= prob_sum;
			    prob_grad += prob_grad_from_logits / prob_sum;
			}

            // My Fix
            float acc_bin_logit = 1.0f - bin_logits[pts_idx];  // accumulated occupancy probability, \prod_{i=1}^n (1 - p_i a_i)
