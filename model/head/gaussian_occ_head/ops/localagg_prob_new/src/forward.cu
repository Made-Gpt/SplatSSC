	...
	float* __restrict__ out_probability)
{
	...
	float C[CHANNELS] = {0};  // accumulate only by valid channels
	float prob_sum = 0;  // accumulate merge probability by channels
	float bin_logit = 1.0;
	float density = 0.0;
	...
	for (int i = range.x; i < range.y; i++)
	{
        ...
        {
            // C[ch] += semantic[CHANNELS * gs_idx + ch] * prob;  // numerator
            C[ch] += semantic[CHANNELS * gs_idx + ch] * prob;  // numerator
            loc_prob += semantic[CHANNELS * gs_idx + ch] * prob;  // denominator
        }

        // My Fix
		bin_logit = (1 - power * opas[gs_idx]) * bin_logit;  // probability of not occupied by gaussian G_i
		prob_sum = loc_prob + prob_sum;
