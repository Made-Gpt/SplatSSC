...
__device__ void bilinear_sampling_grad(
    ...
    const int &num_embeds, const float &h_im, const float &w_im,
    const int &base_ptr,
    const float &grad_output,  // backward propagation grad
    float *&grad_mc_ms_feat, float *grad_sampling_location, float *grad_weights) {
  // get the nearest 4 int grid value of sampling location (h_im, w_im)
  const int h_low = floorf(h_im);
  const int w_low = floorf(w_im);
  const int h_high = h_low + 1;
  const int w_high = w_low + 1;
  ...
  // stride on width and height direction
  // take h=30, w=40 as an example, each 'h' has 40 'w_stride', each 'w_stride' has 96 dim
  const int w_stride = num_embeds;  // (for each width) each point have 'num_embeds' feature value, we must skip 'num_embeds' elements while moving to the next point
  const int h_stride = width * w_stride;  // (for each height) have 'width' point in each row, and each point have 'num_embeds' feature value
  const int h_low_ptr_offset = h_low * h_stride;  // h_low in [0, 29], point to the h_low-th height
  const int h_high_ptr_offset = h_low_ptr_offset + h_stride;
  const int w_low_ptr_offset = w_low * w_stride;  // w_low in [0, 39], point to the w_low-th width
  const int w_high_ptr_offset = w_low_ptr_offset + w_stride;

  const float w1 = hh * hw, w2 = hh * lw, w3 = lh * hw, w4 = lh * lw;
  const float top_grad_mc_ms_feat = grad_output * weight;  // current grad
...
__global__ void deformable_aggregation_kernel(
    ...
    // ms_feat size of each cam is 'num_feat * num_embeds'
    const int value_cam_stride = num_feat * num_embeds;  // token feature number * 96
    const int weight_cam_stride = num_scale * num_groups;  // 4 * 3, num_scale represents the number of multi-scale levels
    // (batch_index * num_pts + pts_index) is the index of pts_index-th sampling point in current batch
    // for 'sampling location'
    int loc_offset = (batch_index * num_pts + pts_index) * num_cams << 1;
    // value_offset points to the channel_index-th channel for 'mc_ms_feat'
    // for 'multi-camera multi-scale feature'
    int value_offset = batch_index * num_cams * value_cam_stride + channel_index;
    // weight_offset points to the groups_index-th group for the 'weight'
    // for 'weight'
    int weight_offset = (
    ...
    for (int cam_index = 0; cam_index < num_cams; ++cam_index, loc_offset += 2) {
        // loc_w and loc_h here are all normalized location
        ...
        if (loc_w > 0 && loc_w < 1 && loc_h > 0 && loc_h < 1) {
            for (int scale_index = 0; scale_index < num_scale; ++scale_index) {
                // scale_offset choose the scale (76800, 9600, 100800) for each channel
                const int scale_offset = scale_start_index[scale_index] * num_embeds;

                const int spatial_shape_ptr = scale_index << 1;  // '<<1' equals to '*2'
                const int h = spatial_shape[spatial_shape_ptr];  // 240 or 120 or 60 or 30
                const int w = spatial_shape[spatial_shape_ptr + 1];  // 320 or 160 or 80 or 40

                // map to pix grid
                const float h_im = loc_h * h - 0.5;
                const float w_im = loc_w * w - 0.5;

                // value_offset decide which channel (3 channel, each have 32 dim), scale_offset locate the specific scale feature of this channel
...
__global__ void deformable_aggregation_grad_kernel(
    ...
    // threadIdx = 0~512, blockDim = 512, blockIdx = 0~n
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_kernels) return;
    const float grad = grad_output[idx];

    const int channel_index = idx % num_embeds;
    const int groups_index = channel_index / (num_embeds / num_groups);  // num_embeds / num_groups = 32
    idx /= num_embeds;
    const int pts_index = idx % num_pts;
    idx /= num_pts;
    const int batch_index = idx;
    ...
    for (int cam_index = 0; cam_index < num_cams; ++cam_index, loc_offset += 2) {
        ...
        if (loc_w > 0 && loc_w < 1 && loc_h > 0 && loc_h < 1) {
            for (int scale_index = 0; scale_index < num_scale; ++scale_index) {
                ...
                const float weight = weights[weights_ptr];  // return a single float value
...
void deformable_aggregation_grad(
  ...
  int num_groups
    ...
    // gridDim = (int)ceil(((double)num_kernels/512)), blockDim=512
}
