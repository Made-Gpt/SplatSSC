...
def main(args):
    ...
    if hasattr(cfg, 'fine_tuning_ckpt'):
        have_fine_tuning_ckpt = True
        print(f"fine tune path: {cfg.fine_tuning_ckpt}")
    else:
        have_fine_tuning_ckpt = False
        print(f"fine tune path: ")
    ...
    if cfg.resume_from and osp.exists(cfg.resume_from):
    ...
    elif have_fine_tuning_ckpt:
        ckpt = torch.load(cfg.fine_tuning_ckpt, map_location='cpu')
        if 'state_dict' in ckpt:
            state_dict = ckpt['state_dict']
        else:
            state_dict = ckpt
        ...
        try:
            ...
            state_dict = revise_ckpt_2(state_dict)
            print(my_model.load_state_dict(state_dict, strict=False))
        print("use pretrained depth fine tuning model")
        ...
        for i_iter, data in enumerate(train_dataset_loader):
            ...
            with torch.cuda.amp.autocast(enabled=amp):
                result_dict = my_model(
                    imgs=imgs, metas=metas, points=None, label=label, grad_frames=cfg.grad_frames, test_mode=False)
                total_anchor_num += result_dict['gaussian'].means.shape[1]
                local_iter_num += 1
            ...
            if i_iter % print_freq == 0 and is_main_process():
                ave_anchor_num = total_anchor_num / local_iter_num
                ...
                logger.info('[TRAIN] Epoch %d Iter %5d/%d   ' % (epoch + 1, i_iter, len(train_dataset_loader)) + loss_info +
                            'GradNorm: %.3f,   lr: %.7f,   time: %.3f (%.3f),   average anchor: %5d' % (grad_norm, lr, time_e - time_s, data_time_e - data_time_s, ave_anchor_num))
                loss_record.reset()
                total_anchor_num = 0
                local_iter_num = 0
        ...
        if epoch % eval_freq == 0:
            ...
            with torch.no_grad():
                for i_iter_val, data in enumerate(val_dataset_loader):
                    ...
                    voxel_predict = result_dict['ce_input'].argmax(dim=1).long()  # [bs, 60, 60, 36]
                    voxel_label = result_dict['ce_label'].long()  # [bs, 60, 60, 36]
                    voxel_fov_mask = result_dict['fov_mask'].long()

                    # voxel_predict[voxel_predict == 0] = 255
                    # voxel_predict[voxel_predict == 12] = 0 # FIXME, 0(baseline): empty, 255: ignore
                    voxel_label[voxel_label == 0] = 255
                    voxel_label[voxel_label == 12] = 0  # FIXME, 0(baseline): empty, 255: ignore
                    # voxel_predict = voxel_predict.cpu()
                    # voxel_label = voxel_label.cpu()

                    # nonempty_mask = voxel_label != 0  # mask empty voxels (according to the label)
                    # CalMeanIou.add_batch(voxel_predict, voxel_label, nonempty_mask)

                    # set both "ignore" label and "empty" label to '0'
                    voxel_predict = voxel_predict.cpu()
