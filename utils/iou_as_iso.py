...
class SSCMetrics:
    ...
    def add_batch(self, y_pred, y_true, nonempty=None, nonsurface=None):
        self.count += 1
        mask = y_true != 255  # mask 'ignored' voxels (according to the label)
        ...
        loc_confusion_matrix = self.get_confusion_matrix(y_pred, y_true, mask)
        self.confusion_matrix += loc_confusion_matrix
    ...
    def get_stats(self):
        ...
        confusion_matrix_norm = self.confusion_matrix / (self.confusion_matrix.sum(axis=1, keepdims=True) + 1e-8)
        return {
            "precision": precision,
            "recall": recall,
            "iou": iou,
    ...
    def reset(self):
        ...
        self.confusion_matrix = np.zeros((self.n_classes, self.n_classes), dtype=np.float32)
    ...
    def get_score_completion(self, predict, target, nonempty=None):
        ...
        _bs = predict.shape[0]  # batch size (1)
        # ---- ignore, map ignored labels to '0' ('empty')
    ...
    def get_score_semantic_and_completion(self, predict, target, nonempty=None):
        ...
        # ---- ignore, set to the same label with 'empty'
        predict[target == 255] = 0
        target[target == 255] = 0
        # ---- flatten
        target = target.reshape(_bs, -1)  # (_bs, 129600)
    ...
    def get_confusion_matrix(self, predict, target, mask=None):
        # predict: [1, 60, 60, 36]
        # target: [1, 60, 60, 36]
        ...
        no_empty_mask = target_flat != 255
        if mask is not None:
            mask = no_empty_mask & mask.reshape(-1)
        else:
            no_ignore_mask = target_flat != 255
            mask = no_empty_mask & no_ignore_mask

        pred_filtered = pred_flat[mask]
        target_filtered = target_flat[mask]

        pred_filtered = np.clip(pred_filtered, 0, self.n_classes - 1)
        target_filtered = np.clip(target_filtered, 0, self.n_classes - 1)

        return confusion_matrix(target_filtered, pred_filtered, labels=range(self.n_classes))

    def print_confusion_matrix(self, save_path=None):

        # Get statistics
        stats = self.get_stats()
        cm = stats["confusion_matrix"]
        cm_norm = stats["confusion_matrix_norm"]

        # Create a list of class names (in index order)
        class_names = [self.class_dict[str(i)] if str(i) in self.class_dict else f"Class_{i}"
        ...
        print(df_cm_norm.round(3).to_string())
        print()
        ...
        for i in range(self.n_classes):
            ...
            print(f"{class_names[i]:<12} {precision:<8.4f} {recall:<8.4f} {f1:<8.4f} {support:<8.0f}")
        ...
        if save_path:
            self.plot_confusion_matrix(cm, cm_norm, class_names, save_path)
